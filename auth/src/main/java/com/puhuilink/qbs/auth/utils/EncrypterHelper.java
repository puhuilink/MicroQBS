...
public final class EncrypterHelper {
    ...
    private EncrypterHelper() {
        ...
        try {
            final KeySpec keySpec = new PBEKeySpec(SecurityConstant.JWT_ENCRYPTER_KEY.toCharArray(), salt, iterationCount);
    ...
    public static String encrypt(String str, String hashField) {
        return instance.encrypt(String.format("%s%s%s", encodeHasField(hashField), str, SecurityConstant.JWT_ENCRYPTER_KEY));
    }
    ...
    public static String decrypt(String encodeStr, String hashField) {
        ...
        decrypt = decrypt.replace(SecurityConstant.JWT_ENCRYPTER_KEY, "");

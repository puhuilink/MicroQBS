...
import com.phlink.bus.api.system.dao.LogMapper;
...
public class LogServiceImpl extends ServiceImpl<LogMapper, SysLog> implements LogService {
    ...
    public IPage<SysLog> findLogs(QueryRequest request, SysLog sysLog) {
        try {
            ...
            this.baseMapper.listLogs(page, sysLog);
            List<SysLog> logs = page.getRecords().stream().peek( l -> {
                if(StringUtils.isNotBlank(l.getUsername())) {
                    User user = userService.findByName(l.getUsername());
                    if(user != null) {
                        if(StringUtils.isNotBlank(user.getRealname())) {
                            l.setUsername(user.getRealname());
                        }
                    }
                }
    ...
    public void deleteLogs(String[] logIds) {
        List<Long> list = Stream.of(logIds).map(Long::parseLong).collect(Collectors.toList());

package com.phlink.bus.api.common.authentication;
...
//
//        String encryptToken = BusApiUtil.encryptToken(token);
//        String encryptTokenInRedis = null;
//        try {
//            encryptTokenInRedis = redisService.get(BusApiConstant.TOKEN_CACHE_PREFIX + encryptToken + StringPool.DOT + ip);
...
//        }
        ...
        try {
            ...
            if(StringUtils.isNotBlank(onlyLogin)) {
                String onlyToken = cacheService.getAppLoginToken(username);
                if(!onlyToken.equals(onlyLogin)) {
                    throw new AuthenticationException("请进行登录");
                }
            }
...
//            throw new AuthenticationException("token校验不通过");
//        }
        return new SimpleAuthenticationInfo(token, token, "busapi_shiro_realm");
    }
}

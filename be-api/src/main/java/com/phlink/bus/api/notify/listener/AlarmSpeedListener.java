...
@Slf4j
@Component
public class AlarmSpeedListener implements ApplicationListener<AlarmSpeedEvent> {
    ...
    @Async
    @Override
    public void onApplicationEvent(AlarmSpeedEvent event) {
        ...
        pushBean.setTitle("车辆超速");
        Date date=new Date(dvrLocation.getGpstime());
        SimpleDateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String dateText = df2.format(date);

        pushBean.setAlert(String.format("中益安达校车%s，司机：%s 在%s发生超速事件，请悉知。", bus.getNumberPlate(), driver.getRealname(), dateText));
